...
def OffpolicyQlearning150816(qldata3, gamma, alpha, numtraces):
    ...
    sumQ = np.zeros(numtraces)  # record sum of Q after each iteration
    # nact = len(np.unique(qldata3[:, 2]))   #nr of actions
    nact = 25  # nr of actions
    # ncl = len(np.unique(qldata3[:, 1]))
    ncl = 752
    Q = np.zeros([ncl, nact])
    maxavgQ = 1
    modu = 100
    listi = np.where(qldata3[:, 0] == 1)[0]  # position of 1st step of each episodes in dataset
    nrepi = len(listi)  # nr of episodes in the dataset
    jj = 0
    ...
    for j in range(numtraces):
        ...
        while qldata3[i + 1, 0] != 1:
            S1 = int(qldata3[i + 1, 1]) - 1
            a1 = int(qldata3[i + 1, 2]) - 1
        ...
        if j % (500 * modu) == 0:  # check if can stop iterating (when no more improvement is seen)
            #      sumQ(jj,1)=sum(sum(Q));
            #      jj=jj+1;
            s = np.mean(sumQ[j - 49999:j])
            d = (s - maxavgQ) / maxavgQ
            if abs(d) < 0.001:
                break  # exit routine
            maxavgQ = s

    sumQ = np.delete(sumQ, np.s_[j:], 0)
...
def offpolicy_eval_tdlearning(qldata3, physpol, gamma, num_iter):
    ...
    for i in tqdm(range(num_iter), desc='offpolicy_ql'):
...
def offpolicy_eval_wis(qldata3, gamma, num_iter):
    ...
    for jj in tqdm(range(num_iter), desc='offpolicy_wis'):
...
def offpolicy_eval_tdlearning_with_morta(qldata3, physpol, ptid, idx, actionbloctrain, Y90, gamma, num_iter):
    ...
    for i in range(num_iter):
        ...
        for ii in range(len(jj)):  # record offline Q value in training set & outcome - for plot
            ...
            prog[jprog, 2] = ptid[jj[
                ii]]  # HERE EACH ITERATION GIVES A DIFFERENT PT_ID  //// if I just do rep*ptid it bugs and mixes up ids, for ex with id3 x rep 10 = 30 (which already exists)
            prog[jprog, 3] = i
            jprog += 1
